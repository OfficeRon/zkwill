...
contract ZKTree is MerkleTreeWithHistory {
    ...
    function _nullify(
        ...
        require(!nullifiers[bytes32(_nullifier)], "The nullifier has been submitted");
        require(isKnownRoot(bytes32(_root)), "Cannot find your merkle root");
        require(
            verifier.verifyProof(
                _proof_a,
                _proof_b,
                _proof_c,
                [_nullifier, _root, isOwner]
            ),
            "Invalid proof"
        );
        nullifiers[bytes32(_nullifier)] = true;
    }
}